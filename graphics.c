...
void init_graphics(int W, int H)
    ...
    window = SDL_CreateWindow( "PF - NEW SDL 2.0", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WIDTH, HEIGHT, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
...
void affiche_all()
	...
	if (__init_graphics_is_already_called == 25) SDL_UpdateWindowSurface(window);
		else {
...
POINT get_arrow(POINT dep)
	{
	//POINT dep;
	//dep.x = dep.y = 0;
	SDL_Event event;
    const Uint8* currentKeyStates = SDL_GetKeyboardState( NULL );
    if( currentKeyStates[ SDL_SCANCODE_UP ] ) {(dep.y) = 1; }
    if( currentKeyStates[ SDL_SCANCODE_DOWN ] ) {(dep.y) = -1; }
    if( currentKeyStates[ SDL_SCANCODE_LEFT ] ) {(dep.x) = -1; }
    if( currentKeyStates[ SDL_SCANCODE_RIGHT ] ) {(dep.x) = 1; }
			...
			switch (event.key.keysym.sym)
					{
						...
						case SDLK_LEFT   : (dep.x) = -1; break;
						case SDLK_RIGHT  : (dep.x) = 1; break;
						case SDLK_UP     : (dep.y) = 1; break;
						case SDLK_DOWN   : (dep.y) = -1; break;
						default          : break;
					}
			}
        */
